...
| 04 | [`fast/04-create-unit-from-template.sh`](../../../tests/e2e/scenarios/fast/04-create-unit-from-template.sh) | *(Exercises a removed endpoint — `POST /api/v1/units/from-template` — and is scheduled for replacement with a package-install scenario.)* |
...
| 07 | [`fast/07-create-start-unit.sh`](../../../tests/e2e/scenarios/fast/07-create-start-unit.sh) | `spring unit create` + `spring unit start` + poll for `Running`/`Starting` status — the lifecycle path you run after first-time setup. |
...
| 13 | [`fast/13-agent-domain-message.sh`](../../../tests/e2e/scenarios/fast/13-agent-domain-message.sh) | Messaging plumbing — `POST /api/v1/messages` to an agent lands a `MessageReceived` activity event, proving router → actor → activity-bus wiring without needing an LLM backend. |
| 14 | [`fast/14-conversation-lifecycle.sh`](../../../tests/e2e/scenarios/fast/14-conversation-lifecycle.sh) | Conversation state machine — a fresh `ConversationId` triggers `MessageReceived` → `ThreadStarted` → `StateChanged (Idle→Active)` in order. Exercises the upstream half of the dispatch loop. |
| 15 | [`fast/15-unit-policy-roundtrip.sh`](../../../tests/e2e/scenarios/fast/15-unit-policy-roundtrip.sh) | Policy CRUD — `GET`/`PUT /api/v1/units/{id}/policy` for the two shipped dimensions (`skill`, `model`): empty → write → read → clear → read, plus 404 on unknown unit. |
| 16 | [`fast/16-cost-api-shape.sh`](../../../tests/e2e/scenarios/fast/16-cost-api-shape.sh) | Cost aggregation API shape — a brand-new agent/unit/tenant each return a well-formed `CostSummary` with zero counters and a valid time window; explicit `from`/`to` overrides are honoured. |
| 17 | [`fast/17-activity-query-filters.sh`](../../../tests/e2e/scenarios/fast/17-activity-query-filters.sh) | Activity query filters — asserts that `source`, `eventType`, `severity`, and `pageSize` on `/api/v1/activity` all narrow results correctly. Covers the query path every observability surface (portal, CLI, dashboard) depends on. |
