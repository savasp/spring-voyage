...
| 14 | [`fast/14-conversation-lifecycle.sh`](../../../tests/e2e/scenarios/fast/14-conversation-lifecycle.sh) | Conversation state machine — a fresh `ConversationId` triggers `MessageReceived` → `ConversationStarted` → `StateChanged (Idle→Active)` in order. Exercises the upstream half of the dispatch loop. |
