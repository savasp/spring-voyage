...
> **Doc-currency banner.** This walkthrough was written against the pre-v2 IA portal (umbrella [#815](https://github.com/cvoya-com/spring-voyage/issues/815)) and several sub-pages have since shifted: the tenant-wide expertise lens moved from `/directory` to `/discovery`; the in-shell **Settings drawer** was replaced by a dedicated `/settings` hub (`/settings/agent-runtimes`, `/settings/packages`, `/settings/skills`, `/settings/system-configuration`); unit detail pages now ride `/units/{name}` (not `/units/{id}`); and the standalone `/agents`, `/conversations`, `/initiative`, and `/system/configuration` top-level routes have been retired or folded into other surfaces. Treat the route table and per-page sections below as a structural sketch — the file/route paths in code references may be stale until the page-by-page rewrite lands. The CLI-equivalent columns are still authoritative because they reference shipped CLI verbs. If you spot a drift, file it on the [V2 IA umbrella](https://github.com/cvoya-com/spring-voyage/issues/815) or against [#406](https://github.com/cvoya-com/spring-voyage/issues/406).
...
- **Repository** — single dropdown of `{owner}/{repo}` rows aggregated across every GitHub App installation visible to the **signed-in GitHub user** (sourced from `GET /api/v1/connectors/github/actions/list-repositories`, which uses the user's OAuth token to call `GET /user/installations` + `GET /user/installations/{installation_id}/repositories`). Picking a row carries its installation id along with `owner` and `repo`, so the operator never has to discover or paste an installation id by hand. Private repositories are tagged inline. If the operator hasn't signed in with GitHub yet the wizard renders a "Sign in with GitHub" CTA that kicks off the OAuth flow; the callback redirects back into the wizard with the session id in the URL fragment. If the list is empty after sign-in, the portal shows an "Install App" banner with a deep-link to the platform's install URL (`/api/v1/connectors/github/install-url`). ([#1153](https://github.com/cvoya-com/spring-voyage/issues/1153))
...
- **Webhook events** — pick the event set the unit subscribes to. The wizard surfaces a **Connector defaults** checkbox followed by an informational row of per-event checkboxes (issues, pull_request, issue_comment, push, release). While **Connector defaults** is checked, the per-event row reflects the connector's defaults (issues, pull_request, issue_comment) and is disabled — the wizard sends no `events` field on the wire so the server resolves the set itself. Unchecking **Connector defaults** enables the per-event row and pre-populates it with the same defaults as a starting point. ([#1127](https://github.com/cvoya-com/spring-voyage/issues/1127))
