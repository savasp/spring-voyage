...
- **GitHub App installation enumeration is user-scoped by default.** The repository dropdown surfaced by the create-unit wizard (and the unit's Connector tab) always runs through the operator's GitHub identity, even on self-hosted single-tenant deployments. The flow is:
  1. The wizard asks the operator to sign in with GitHub via the OAuth flow exposed at `GET /api/v1/connectors/github/oauth/authorize` — the OAuth callback redirects back to the wizard with the `oauth_session_id` in the URL fragment so the session id never appears in server logs or `Referer` headers.
  2. `GET /api/v1/connectors/github/actions/list-repositories` resolves the user OAuth token via the injected `IGitHubUserAccessTokenProvider`, then calls `GET /user/installations` and `GET /user/installations/{installation_id}/repositories` against the operator's token. The response is the intersection of (installations the configured GitHub App is part of) ∩ (repositories the signed-in user can see).
  3. If no signed-in GitHub user is present, the endpoint refuses to fall back to the App-wide enumeration. It returns `401 Unauthorized` with a structured `requires_signin: true` extension so the wizard renders a "Sign in with GitHub" CTA. The previous App-scoped path (`GET /app/installations` + `GET /installation/{id}/repositories` with the App JWT) is no longer reachable from the wizard surface.
  4. Cloud / multi-tenant deployments swap `IGitHubUserAccessTokenProvider` to resolve the token from their own session/identity store. The OSS default reads the `oauth_session_id` query parameter or `X-GitHub-OAuth-Session` header and looks the token up via `IOAuthSessionStore` + `ISecretStore`.

  The portal's create-unit wizard never exposes a raw "App installation" picker — installation ids ride along on each repository row, so the only enumeration the operator ever sees is repositories they could reach in GitHub directly. ([#1133](https://github.com/cvoya-com/spring-voyage/issues/1133), [#1153](https://github.com/cvoya-com/spring-voyage/issues/1153))
